...
\subsection{Resultados econométricos y estimación del modelo final}
...
\toprule
\textbf{Variables Independientes} & \textbf{Modelo (1)} & \textbf{Modelo (2)} & \textbf{Modelo (3)} & \textbf{Modelo (4) Robusto} & \textbf{Modelo (5) Robusto} \\
\midrule
Semillas certificadas ($X_1$) & 0.2509*** & 0.0266 & 0.0202 & 0.0982*** & {0.0982***} \\
                            & (0.0253)  & (0.0226) & (0.0224) & (0.0234)  & {(0.0234)} \\
Fuente de agua              &           & 1.1527*** & 1.1311*** & 0.4858*** & {0.4858***} \\
                            &           & (0.0422) & (0.0422) & (0.0872)  & {(0.0871)} \\
Sequía                      &           &          & -0.0593  & -0.0818*  & {-0.0818*} \\
                            &           &          & (0.0459) & (0.0492)  & {(0.0492)} \\
Lluvias a destiempo         &           &          & -0.2874*** & -0.1968* & {-0.1968*} \\
                            &           &          & (0.0718) & (0.1025)  & {(0.1025)} \\
Plagas y enfermedades       &           &          & -0.1307*** & -0.1323*** & {-0.1323***} \\
                            &           &          & (0.0209) & (0.0200)  & {(0.0200)} \\
Otros factores climáticos   &           &          & -0.4079*** & -0.3629*** & {-0.3629***} \\
                            &           &          & (0.0612) & (0.0625)  & {(0.0625)} \\
...
Amazonas                    &           &          &          & 0.0738*   & 0.0738* (0.0443) \\
Áncash                      &           &          &          & 0.4436*** & 0.4436*** (0.0468) \\
Arequipa                    &           &          &          & 0.4987*** & 0.4987*** (0.0412) \\
Ayacucho                    &           &          &          & -0.7023*** (0.1037) & [Depurado] \\
Cajamarca                   &           &          &          & 0.1531*** & 0.1531*** (0.0503) \\
Huánuco                     &           &          &          & -0.2049*  & -0.2049* (0.1085) \\
Junín                       &           &          &          & -0.8428*** & -0.8428*** (0.2387) \\
La Libertad                 &           &          &          & 0.3657*** & 0.3657*** (0.0411) \\
Lambayeque                  &           &          &          & 0.0975**  & 0.0975** (0.0406) \\
Loreto                      &           &          &          & -0.2290   & -0.2290 (0.1453) \\
Madre de Dios               &           &          &          & -0.6204*** & -0.6204*** (0.1295) \\
Pasco                       &           &          &          & -0.5369*** & -0.5369*** (0.1465) \\
Piura                       &           &          &          & 0.0467    & 0.0467 (0.0440) \\
San Martín (Base)           &           &          &          & 0.0000 (.) & 0.0000 (.) \\
Tumbes                      &           &          &          & 0.1179*   & 0.1179* (0.0610) \\
Ucayali                     &           &          &          & -0.3421*** & -0.3421*** (0.0614) \\
\addlinespace[0.1cm]
Constante ($\hat{\beta}_0$) & 8.7623*** & 7.8895*** & 7.8169*** & 8.2564*** & {8.2564***} \\
...
\bottomrule
\end{tabular*}
\vspace{2pt}\par\noindent{\scriptsize \textit{Nota.} Errores estándar entre paréntesis (\texttt{vce(robust)} reportados para los Modelos 4 y 5, justificando la corrección de heterocedasticidad). Nivel de significancia: *** $p<0.01$, ** $p<0.05$, * $p<0.1$. Los Modelos 1-3 se estiman por MCO estándar secuencialmente ($N=1,992$). El Modelo (4) evalúa el modelo completo sobre el total de parcelas ($N=1,992$), mientras que el Modelo (5) excluye el grupo \textit{singleton} de Ayacucho ($N=1,991$) conforme a \citet{correia2015}. Todos los modelos con efectos fijos regionales toman a San Martín como categoría base (\texttt{ib14.departamento}).\par}
\end{table*}

\subsubsection{Especificación funcional del modelo final y resultados empíricos}
...
\subsubsection{Impacto productivo de las semillas certificadas ($X_1$)}
El parámetro estimado para nuestra variable de interés fundamental, las semillas certificadas ($X_1$), en el modelo econométrico definitivo con errores estándar robustos a heterocedasticidad (Modelo 5, $N=1,991$), arrojó un coeficiente de $\hat{\beta}_1 = 0.0982$ ($p < 0.001$, con un error estándar robusto de $0.0234$ y un estadístico $t = 4.19$). Al tratarse de una especificación semilogarítmica ($\ln Y$), la transformación exacta semielástica para variables dicotómicas propuesto por \citet{halvorsen1980} es:
\begin{equation}
\scriptsize
\begin{split}
    \Delta\% \text{ Rendimiento} &= \left[ \exp(\hat{\beta}_1) - 1 \right] \times 100 \\
    &= \left[ \exp(0.0982) - 1 \right] \times 100 = +10.32 \%
\end{split}
\end{equation}
En términos agronómicos y económicos, este hallazgo empírico demuestra que, manteniendo constantes la disponibilidad hídrica, la incidencia de choques climáticos, el capital humano del productor y la heterogeneidad regional permanente, una parcela arrocera en el Perú que adopta semillas certificadas de alta calidad obtiene, en promedio, un rendimiento físico por hectárea $10.32$\% superior frente a una parcela que emplea semillas no certificadas o granos comerciales comunes ($p < 0.001$). Este resultado valida de forma concluyente la hipótesis central de la investigación y respalda las directrices técnicas de la \citet{fao2019} sobre el impacto crítico del vigor germinativo y la pureza varietal en la productividad del cultivo.

\subsubsection{Comparación metodológica entre los Modelos (4) y (5): depuración del \textit{singleton} de Ayacucho}
El análisis comparativo entre la columna del Modelo 4 Robusto (muestra completa, $N=1,992$) y el Modelo 5 Robusto (muestra purificada, $N=1,991$) de la \hyperref[tab:resultados]{Tabla~\ref*{tab:resultados}} ilustra de manera empírica el problema econométrico formalizado por \citet{correia2015} respecto a las observaciones unitarias (\textit{singletons}) en modelos de efectos fijos con errores robustos. En primer lugar, respecto a la invarianza en las estimaciones puntuales, se constata que ningún coeficiente de las variables independientes varía empíricamente entre ambos modelos; el parámetro de la semilla certificada permanece idéntico en $0.0982$, el del agua en $0.4858$ y los choques climáticos mantienen exactamente sus mismas pendientes y errores estándar robustos al cuarto decimal. En segundo lugar, respecto a la fundamentación matemática del comportamiento del \textit{singleton}, en la base de microdatos ENA 2024 el departamento de Ayacucho registra una única unidad agropecuaria arrocera encuestada ($N_{\text{Ayacucho}} = 1$). Al especificar el vector de efectos fijos departamentales por MCO (\texttt{ib14.departamento}), la variable indicadora creada para Ayacucho ($\hat{\gamma}_{\text{Ayacucho}} = -0.7023$ en el Modelo 4) actúa absorbiendo de forma perfecta el intercepto de dicha parcela. Por consiguiente, su residuo exacto es idénticamente cero ($e_{\text{Ayacucho}} = 0$) y la parcela no aporta ninguna variación transversal intra-grupo para identificar los coeficientes de las demás covariables.

En tercer lugar, respecto a los efectos de la depuración sobre la inferencia del Modelo 5 ($N=1,991$), conservar un \textit{singleton} incrementa espuriamente el número de observaciones de $N=1,991$ a $N=1,992$ sin aportar un verdadero grado de libertad libre. En estimaciones con corrección de Huber-White (\texttt{vce(robust)}), ese grado de libertad ficticio distorsiona el factor de ajuste de muestras finitas $\frac{N}{N-k}$. Al depurar Ayacucho —en concordancia con el estándar computacional de paquetes como \texttt{reghdfe}—, el estadístico global $F(23, 1967) = 76.32$ ($p < 0.0001$) y los errores estándar reportados reflejan una inferencia estadística exacta y libre de sobreparámetros. Asimismo, esto genera un sinceramiento en el $R^2$ structural del modelo, dado que en el Modelo 4 el $R^2$ asciende a $0.5502$ ($R^2$ aj. $0.5447$), mientras que al eliminar el \textit{singleton} en el Modelo 5 el $R^2$ pasa a $0.5480$ ($54.80$\%) y el ajustado a $0.5428$. Esta pequeña corrección elimina la bondad de ajuste espuria derivada de que la variable indicadora de Ayacucho explicaba artificialmente el 100\% de su propia observación, consolidando al $54.80$\% como el verdadero poder explicativo sobre las parcelas con variabilidad muestral válida.

\subsubsection{Controles agronómicos, climáticos y heterogeneidad territorial}
Las variables de control incorporadas en la especificación actúan eficazmente para aislar el impacto puro de $X_1$ y muestran un comportamiento acorde con la teoría agraria. En cuanto al riego artificial, la disponibilidad hídrica constituye el factor productivo individual más potente del modelo, incrementando la cosecha en $+62.55$\% ($\hat{\beta} = 0.4858$, $p < 0.001$), lo que ratifica los postulados de la \citet{fao2011} sobre el agua como insumo restrictivo. Por su parte, los choques climáticos adversos penalizan de manera estadísticamente significativa el rendimiento, conforme a \citet{schlenker2009}; en particular, las plagas y enfermedades reducen la producción en $-12.39$\% ($\hat{\beta} = -0.1323$, $p < 0.001$), las lluvias fuera de temporada en $-17.86$\% ($\hat{\beta} = -0.1968$, $p < 0.05$), las sequías en $-7.86$\% ($\hat{\beta} = -0.0818$, $p < 0.10$) y otros factores climáticos extremos en $-30.43$\% ($\hat{\beta} = -0.3629$, $p < 0.001$). Respecto al capital humano y la demografía, la conducción por una mujer productora exhibe una brecha del $-7.07$\% ($\hat{\beta} = -0.0733$, $p < 0.001$), reflejando desigualdades estructurales de acceso a crédito e insumos \citep{aguilar2015}, mientras que la educación superior incrementa la productividad en $+6.93$\% ($\hat{\beta} = 0.0670$, $p < 0.001$) y la edad del productor presenta un parámetro nulo ($\hat{\beta} = 0.0001$, $p = 0.807$). En cuanto a los efectos fijos departamentales, tomando a San Martín como base, los valles costeros con alta tecnificación de riego como Arequipa ($\hat{\gamma} = 0.4987$, $+64.66$\%), Áncash ($+55.83$\%) y La Libertad ($+44.15$\%) arrojan ventajas agroecológicas masivas ($p < 0.001$), mientras que las regiones marginales o dependientes de lluvias enfrentan penalizaciones severas como Junín ($\hat{\gamma} = -0.8428$, $-56.96$\%), Madre de Dios ($-46.23$\%) y Loreto ($-20.47$\%). Las pruebas de significancia conjunta de Wald contrastaron empíricamente la relevancia global de estos bloques en el Modelo 5 ($df = 1967$), arrojando $F(4, 1967) = 21.56$ ($p < 0.0001$) para el bloque climático, $F(3, 1967) = 6.62$ ($p = 0.0002$) para el bloque demográfico y el estadístico más alto de $F(14, 1967) = 63.65$ ($p < 0.0001$) para los efectos fijos departamentales, demostrando que absorber la heterogeneidad geográfica resulta indispensable \citep{escobal2002}.

\subsubsection{Pruebas post-estimación y endogeneidad conceptual de las semillas certificadas ($X_1$)}
La validez estadística y especificación del modelo se verificó mediante tres pruebas de diagnóstico formal registradas en el archivo de registro. En primer lugar, el análisis de multicolinealidad mediante el Factor de Inflación de Varianza arroja un $VIF$ promedio de apenas $2.06$, y para la variable de interés semillas certificadas es de $VIF = 1.28$ (equivalente a una tolerancia de $0.78$), lo que al situarse muy por debajo del umbral patológico ($VIF \ge 10$) aprueba la ortogonalidad de $X_1$ respecto al riego y la geografía. En segundo lugar, el test de heterocedasticidad de Breusch-Pagan y Cook-Weisberg evaluado sobre los residuos MCO arroja un estadístico $\chi^2(1) = 108.20$ ($p < 0.0001$); al rechazarse rigurosamente la hipótesis nula de homocedasticidad, se justifica como condición indispensable la corrección con errores estándar robustos de Huber-White (\texttt{vce(robust)}) en los Modelos 4 y 5 para resguardar la inferencia estadística. En tercer lugar, la significancia de Wald evaluada sobre el Modelo 5 corrobora que el efecto aislado de $X_1$ es altamente significativo ($F(1, 1967) = 17.59$, $p < 0.0001$) y que el conjunto total de controles aporta un estadístico robusto de $F(9, 1967) = 17.76$ ($p < 0.0001$).

Respecto a la endogeneidad conceptual de la semilla certificada, en la estimación MCO transversal condicional se asume exogeneidad ($E[\varepsilon_i | X_i] = 0$). Sin embargo, desde la teoría microeconómica de la innovación agrícola \citep{feder1985, alston2021}, se debe asumir conceptualmente que la decisión de utilizar semilla certificada ($X_1$) es una variable endógena ($\text{Cov}(X_{1i}, \varepsilon_i) \neq 0$). Dado que en la encuesta ENA no se dispone de un instrumento estrictamente excluyente para una prueba de Hausman-Wu formal, se fundamenta teóricamente la endogeneidad a partir de tres fuentes clásicas. La primera es la autoselección y heterogeneidad inobservada, puesto que los agricultores no eligen semillas al azar; aquellos que compran semilla certificada suelen poseer una mayor habilidad empresarial intrínseca, motivación por la innovación, mejores prácticas de fertilización o parcelas con mayor calidad micro-local de suelo no capturadas por la encuesta. Si esta habilidad inobservada ($A_i \in \varepsilon_i$) se correlaciona positivamente con $X_1$, el estimador MCO ($\hat{\beta}_1 = +10.32$\%) incurriría en un sesgo al alza. La segunda es la simultaneidad y el efecto riqueza, en la medida en que agricultores que obtuvieron cosechas extraordinarias en campañas pasadas generaron mayores utilidades, lo que redujo sus restricciones de liquidez y les permitió pagar el sobreprecio de la semilla certificada en la campaña en curso ($Y_{i,t-1} \rightarrow X_{1,t} \rightarrow Y_{i,t}$). La tercera es el error de medición en el auto-reporte de encuestas declarativas masivas como la ENA, donde un productor podría clasificar erróneamente como certificada una semilla comercial de buena apariencia estética (o viceversa), lo cual de ser un error clásico generaría un sesgo de atenuación hacia cero. Por consiguiente, al asumir la endogeneidad de $X_1$, el $+10.32$\% obtenido en el Modelo 5 debe interpretarse como una asociación paramétrica condicional robusta de alta precisión en corte transversal ($N=1,991$), aislando el efecto del agua, el clima y las brechas regionales provinciales.
